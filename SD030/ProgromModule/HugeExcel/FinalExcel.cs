...
namespace HugeExcel
{
    public class FinalExcel
    {
        ...
        private void FillBaseData_2022(ExcelPackage package, DataTable dt)
        {
            var sheet = package.Workbook.Worksheets["表-数量"];
            var addr = sheet.Dimension.Address;
        ...
        //图-半钢胎 //图-全钢胎
        private void SetMainStyleAndFormula_2022(ExcelPackage package, ExcelWorksheet sheet)
        {
            int FirstMonthRow = 7;
        ...
        public void Run(DataTable dt)
        {
            if (!string.IsNullOrEmpty(_FilePath))
            {
                ...
                using (ExcelPackage package = new ExcelPackage(new FileInfo(_FilePath)))
                {
                    ...
                    FillBaseData_2022(package, dt);
                    var sheet = package.Workbook.Worksheets["图-半钢胎"];
                    SetMainStyleAndFormula_2022(package, sheet);
                    sheet = package.Workbook.Worksheets["图-全钢胎"];
                    SetMainStyleAndFormula_2022(package, sheet);
                    package.Save();
                }
